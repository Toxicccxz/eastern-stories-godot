...
func _test_input_focus_cancel_and_bleed(tree: SceneTree) -> void:
	...
	_send_action(&"ui_accept")
	await _wait_frames(tree, 3)
	_assert_eq(shell.shell_state().mode(), ApplicationShellState.Mode.NEW_GAME_SETUP, "ui_accept opens character entry")
	PublicNewGameTestFixture.complete_setup(shell)
	await _wait_frames(tree, 3)
	_assert_eq(shell.shell_state().mode(), ApplicationShellState.Mode.PLAYING, "submitted entry starts exactly one New Game")
...
func _test_native_gamepad_navigation(tree: SceneTree) -> void:
	...
	await _send_joy(tree, JOY_BUTTON_A)
	_assert_eq(shell.shell_state().mode(), ApplicationShellState.Mode.NEW_GAME_SETUP, "native A opens setup")
	PublicNewGameTestFixture.complete_setup(shell)
	await _wait_frames(tree, 4)
...
func _test_settings_and_game_save_separation(tree: SceneTree) -> void:
	...
	_assert_true(PublicNewGameTestFixture.request(shell), "corrupt settings cannot block New Game")
	await _wait_frames(tree, 3)
...
func _test_transition_input_and_save_isolation(tree: SceneTree) -> void:
	...
	shell.dismiss_current_result()
	shell.request_return_to_main_menu()
	PublicNewGameTestFixture.confirm(shell)
	await _wait_frames(tree, 4)
