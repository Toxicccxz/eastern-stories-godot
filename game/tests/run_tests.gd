...
func _init() -> void:
	...
	for bf1_suite: Script in [BeastDerivedValuesTest, BeastInitializationTest, SerpentDefinitionTest]:
	...
	var snow_food_result: Dictionary = await load("res://tests/runtime/snow_dumpling_test.gd").new().run_all(self)
	assertion_count += int(snow_food_result["assertions"])
	failures.append_array(snow_food_result["failures"])
	var snow_finance_result: Dictionary = await load("res://tests/runtime/snow_finance_test.gd").new().run_all(self)
	var snow_bank_result: Dictionary = await load("res://tests/runtime/snow_bank_access_test.gd").new().run_all(self)
	assertion_count += int(snow_bank_result["assertions"])
	failures.append_array(snow_bank_result["failures"])
	assertion_count += int(snow_finance_result["assertions"])
	failures.append_array(snow_finance_result["failures"])
	...
	var hockshop_result: Dictionary = await load("res://tests/application/hockshop_core_test.gd").new().run_all(self)
	assertion_count += int(hockshop_result["assertions"])
	failures.append_array(hockshop_result["failures"])
	...
	var martial_result: Dictionary = await load("res://tests/runtime/snow_martial_progression_test.gd").new().run_all(self)
	assertion_count += int(martial_result["assertions"])
	failures.append_array(martial_result["failures"])
