...
func _init() -> void:
	...
	print("RUN Phase 4A1 equipment regression")
	results.append(EquipmentStateTest.new().run_all())
	print("RUN Phase 5B1 combat foundation")
	...
	if failures.is_empty():
		print("PASS Phase 5B1 targeted: %d assertions" % assertions)
		quit(0)
		return
	for failure: String in failures:
		printerr(failure)
	printerr(
		"FAIL Phase 5B1 targeted: %d failure(s), %d assertions"
