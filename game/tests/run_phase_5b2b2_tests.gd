...
func _init() -> void:
	...
	print("RUN Phase 2A internal resource regression")
	results.append(CharacterRecoveryTest.new().run_all())
	print("RUN Phase 3A skill regression")
	results.append(SkillCoreTest.new().run_all())
	print("RUN Phase 3B2 practice/selflearn regression")
	results.append(PracticeSelfLearningTest.new().run_all())
	print("RUN Phase 3B3 skill improvement effects regression")
	results.append(SkillImprovementEffectsTest.new().run_all())
	...
	print("RUN Phase 5B2B1 standard force hit policy")
	results.append(StandardForceHitPolicyTest.new().run_all())
	print("RUN Phase 5B2B2 progression and busy completion")
	...
	if failures.is_empty():
		print("PASS Phase 5B2B2 targeted: %d assertions" % assertions)
		quit(0)
		return
	for failure: String in failures:
		printerr(failure)
	printerr(
		"FAIL Phase 5B2B2 targeted: %d failure(s), %d assertions"
