...
func _test_substantial_round_trip_and_determinism() -> void:
	...
	_assert_eq(decoded.snapshot.items.schema_version, 3, "embedded item schema stays independently versioned")
	...
	_assert_eq(decoded.snapshot.items.schema_version, 3, "embedded item snapshot getter is defensive")
