...
func _test_stable_order_and_lifecycle_staging(tree: SceneTree) -> void:
	...
	_assert_true(controller.enemy_binding.state.vitality.current < 0, "player opportunity crosses enemy threshold before enemy gate")
	...
	_assert_true(controller.lifecycle_is_pending(), "controller records lifecycle pending")
	_assert_true(controller.opportunity_timer.is_stopped(), "lifecycle pending stops encounter cadence")
	_assert_eq(controller.enemy_binding.life_status, CombatSliceLifeStatus.Value.ACTIVE, "6B2 does not execute life-status mutation")
	_assert_eq(controller.enemy_binding.relationship.opponent_ids(), relationship_before, "6B2 does not perform lifecycle relationship cleanup")
	_assert_true(controller.hud.log_lines().back().contains("Lifecycle pending"), "HUD honestly reports staged lifecycle")
	var later: Array[CombatSliceOpportunityResult] = controller.process_cadence_tick()
	_assert_true(later.is_empty(), "paused lifecycle staging executes no later opportunities")
	controller.queue_free()
	await tree.process_frame
...
func _test_player_threshold_waits_for_next_opportunity(tree: SceneTree) -> void:
	...
	_assert_eq(next_tick.size(), 1, "next player lifecycle result stops later enemy participant")
	...
	_assert_eq(controller.enemy_binding.state.vitality.current, enemy_vitality_before_next_tick, "staged player lifecycle permits no later enemy opportunity")
	_assert_eq(controller.player_binding.life_status, CombatSliceLifeStatus.Value.ACTIVE, "player lifecycle remains staged, not executed")
	controller.queue_free()
	await tree.process_frame
