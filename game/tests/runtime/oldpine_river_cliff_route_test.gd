...
func _test_direct_pine_shortcut_and_route_collisions(tree: SceneTree) -> void:
	...
	_assert_eq(outdoor.player_runtime().world_location().zone_id, OldPineWorldDefinitions.RIVER_GORGE_ZONE_ID, "direct displacement awaits zone observation")
	_assert_true(await _walk(outdoor.player_body, Vector2(1420, 2000), tree), "player can return north from Lake")
