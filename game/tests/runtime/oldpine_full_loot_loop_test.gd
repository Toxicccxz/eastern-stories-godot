...
func _test_full_loot_inventory_equip_second_fight_loop(
	...
	if controller == null:
		return
	_assert_eq(_count_tree_nodes(controller), 123, "persisted scene contains Phase 8B2 inventory hierarchy")
...
func _test_fresh_scene_reset_baseline(tree: SceneTree) -> void:
	...
	_assert_eq(fresh.npc_runtimes().size(), 3, "fresh/reset boundary restores three bandits")
...
func _instantiate_scene(tree: SceneTree) -> OldPineOutdoorController:
	var controller: OldPineOutdoorController = (
		SceneType.instantiate() as OldPineOutdoorController
	)
	if controller != null:
