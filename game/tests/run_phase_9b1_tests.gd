...
func _run_tests() -> void:
	...
	print("RUN Phase 9B1 Pine Maze + Tall Bandit")
	...
	if failures.is_empty():
		print("PASS Phase 9B1 targeted regressions: %d assertions" % assertions)
		quit(0)
		return
	for failure: String in failures:
		printerr(failure)
	printerr("FAIL Phase 9B1: %d failure(s), %d assertions" % [failures.size(), assertions])
	quit(1)
