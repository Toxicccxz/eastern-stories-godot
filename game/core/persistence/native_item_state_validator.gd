...
static func validate(
	...
	for record: NativeLiquidConsumableRecord in snapshot.liquid_consumable_records:
		if record == null or not items.has(record.item_instance_id):
			return _failure(ResultType.Outcome.INVALID_LIQUID_RECORD)
		if liquid_ids.has(record.item_instance_id):
			return _failure(ResultType.Outcome.DUPLICATE_LIQUID_RECORD, record.item_instance_id)
		var item: NativeItemRecord = items[record.item_instance_id]
	...
	for id: StringName in items:
		if definitions.liquid_definition(items[id].item_definition_id) != null and not liquid_ids.has(id):
			return _failure(ResultType.Outcome.MISSING_LIQUID_STATE, id)
